...
The CLI `ghcib source <Module>` sends a `SourceQuery` over the socket and writes the
result to stdout, identical to how `ghcib status` works today.

### Formatter (stretch)

If `source-formatter` is set in `.ghcib.toml`, the daemon pipes the stripped source
...
### Ship a standalone `ghcib-source` binary
