...
This proposal adds a `source` subcommand to ghcib and a corresponding socket query type
...
only supply module names. A batch form (`ghcib source M1 M2 ...`) is supported from the
...
- `ghcib source <Module.Name> [Module.Name ...]` CLI subcommand (one or more modules)
...
- `ghcib source Data.Map.Strict` returns the full source of that module.
- `ghcib source Data.Map.Strict Data.Map.Lazy` returns both, clearly separated.
...
- `ghcib source <Module> [Module ...]` CLI subcommand wired end-to-end

**Acceptance criteria:**
`ghcib source Data.Map.Strict` prints the Haskell source of that module to stdout.
`ghcib source Data.Map.Strict Data.Map.Lazy` prints both, separated by a header line.
...
**Deliverables:**
- `SourceQuery` and `SourceResponse` types in `Ghcib.Socket.Protocol`
...
**Deliverables:**
- Optional `source-formatter` field in `.ghcib.toml` (e.g. `"ormolu"` or a full command)
- If configured, the daemon pipes stripped source through the formatter before returning
- CLI respects the same config

**Acceptance criteria:**
With `source-formatter = "ormolu"` set, `ghcib source Data.Map.Strict` returns
