module Unit.Tricorder.WatchSpec (spec_Watch) where
...
import Tricorder.Effects.Display (Style)
import Tricorder.Render (buildStateDoc, daemonInfoDoc, diagnosticBlock, diagnosticDoc)
...
spec_Watch = do
    ...
    describe "daemonInfoDoc" do
        ...
        it "shows socket path" do
            let di = emptyDaemonInfo {sockPath = "/run/user/1000/tricorder/abc.sock"}
            render (daemonInfoDoc di) `shouldContain` "/run/user/1000/tricorder/abc.sock"
        ...
        it "shows log file path when configured" do
            let di = emptyDaemonInfo {logFile = Just "/tmp/tricorder.log"}
            render (daemonInfoDoc di) `shouldContain` "/tmp/tricorder.log"
  where
